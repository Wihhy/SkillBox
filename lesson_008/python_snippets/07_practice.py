...
trucks = []
for guessed_number in range(5):
    truck = Truck(model='КАМАЗ #{}'.format(guessed_number), body_space=5000)
    moscow.truck_arrived(truck)
    trucks.append(truck)
for guessed_number in range(5):
    truck = OtherTruck(model='Volvo #{}'.format(guessed_number), body_space=10000)
    moscow.truck_arrived(truck)
    trucks.append(truck)
